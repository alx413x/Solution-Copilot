...
面向售前工程师的可追溯方案工作台。S00–S06 已通过本地阶段验收；S07 方案生成与编辑、S08 校验与导出、S09 端到端验收的实现与自动/真实链路证据已完成，处 review 状态，详见 [S09 验收记录](docs/S09_VALIDATION.md)。
...
完成 S06 编排后，从“澄清与编排”进入“方案工作台”，采用该编排结果创建项目方案。先生成或人工调整大纲，可增删和排序最多 20 章；确认后系统通过现有 Worker 逐章生成正文。也可以单独生成或重新生成一章，其他章节和历史版本保持不变。

正文使用 Tiptap 编辑器，支持段落、二/三级标题、列表、引用、代码块、横线、简单表格及基础文字样式。当前不支持链接、图片或粘贴 HTML；表格仅支持简单矩形表（最多 50 行、20 列，无合并单元格或自定义列宽）。每章最多 100000 字符、2000 节点、深度 12。人工保存会创建新版本，并把该章既有引用标为待重新核实。版本列表提供历史只读查看，每个方案最多 200 个版本。

生成会把已确认需求和本次授权检索结果发送到已配置的 DeepSeek。引用的 `partial` 状态只表示模型摘录能在来源中逐字找到，仍需人工判断主张是否被来源支持。资料停用或删除后历史摘录继续保留，但原文件链接仍按当前权限和资料状态拒绝访问。

首次更新运行 `uv run alembic upgrade head` 并重启开发服务。自动、真实链路与端到端验收证据见 [S07](docs/S07_VALIDATION.md)、[S08](docs/S08_VALIDATION.md)、[S09](docs/S09_VALIDATION.md) 验收记录。生产 OIDC、部署和完整无障碍审计仍属于后续阶段。

## S08 校验与导出

在方案页对当前不可变版本启动质量检查，结果包含规则 ID、严重程度、问题章节、说明与建议，并给出需求覆盖率；问题可定位到章节。校验为辅助审核，引用摘录匹配不等于语义支持，方案或需求修改后需重新检查。

导出支持 Markdown 和 DOCX，保留章节顺序、编辑器结构、章内引用编号、待核实标记及文末参考资料。DOCX 使用统一标题与正文样式，表格导出为带边框表格。

下载按组织/项目私有路径存储，每次请求重新校验权限并校验 SHA-256，响应 `private, no-store`；撤销授权后旧地址立即失效。仅支持 Markdown/DOCX，不含 PDF 或品牌模板。
